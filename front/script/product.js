...
function main() {
    ...
    const dataApi = fetch(`http://localhost:3000/api/products/${id}`).then(async (resData) => {
        buildProduct(await resData.json(), id);
    });
}

//Insertion des données dans la page product.html
function buildProduct(res, id) {
    // IMAGE
